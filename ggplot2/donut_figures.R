...
single_ring_renewable_donut_p2 <- single_ring_donut_figure_p2(renewable_ring,"Renewable Generation","2.6% in 2019","30% by 2030","label",c("whitesmoke","skyblue","steelblue"))
...
single_ring_carbon_free_donut_p2 <- single_ring_donut_figure_p2(carbon_free_ring,"Carbon-Free Generation","32.9% in 2019","100% by 2050","label",c("seagreen","mediumseagreen"))
...
VCEA_single_ring_sw_capacity_donut_p <- plot_ly(textinfo="none",hoverinfo="label+value") %>%
  add_pie(data = sw_ring2, values = ~value, labels = ~category, sort = F,hole = 0.7,
          ...
          marker=list(colors=c("maroon","lightcoral","indianred"),
...
VCEA_single_ring_sw_capacity_donut_p2 <- single_ring_donut_figure_p2(sw_ring2,"Onshore Wind & Solar Capacity","807.65 MW of Solar Capacity as of 2019","16,100 MW of Onshore Wind & Solar Capacity by 2024","label+value",c("maroon","lightcoral","indianred"))
