...
total_production_mwh <- get_EIA_series(eiaKey,series_id="ELEC.GEN.ALL-VA-99.A")
hydro_production_mwh <- get_EIA_series(eiaKey,series_id="ELEC.GEN.HYC-VA-99.A")
utility_solar_production_mwh <- get_EIA_series(eiaKey,series_id="ELEC.GEN.SUN-VA-99.A")
all_solar_production_mwh <- get_EIA_series(eiaKey,series_id="ELEC.GEN.TSN-VA-99.A")
distrib_solar_production_mwh <- get_EIA_series(eiaKey,series_id="ELEC.GEN.DPV-VA-99.A")
nuclear_production_mwh <- get_EIA_series(eiaKey,series_id="ELEC.GEN.NUC-VA-99.A")

names(total_production_mwh)[2]<- "total_energy_production"
names(hydro_production_mwh)[2]<- "hydro_production"
names(utility_solar_production_mwh)[2]<- "utility_solar_production"
names(all_solar_production_mwh)[2]<- "all_solar_production"
names(distrib_solar_production_mwh)[2]<- "distrib_solar_production"
names(nuclear_production_mwh)[2] <- "nuclear_production"

# Inserting missing values into all solar data
year<-2001:2013
all_solar_production<-rep(0,13)
missing_all_solar_dt<-data.frame(year,all_solar_production)
all_solar_production_mwh<-rbind(all_solar_production_mwh,missing_all_solar_dt)
all_solar_production_mwh<-all_solar_production_mwh[order(-year)]

# Merging hydro, all solar, and nuclear with total energy production
merged_table<-merge(total_production_mwh,hydro_production_mwh,id=year)
VA_annual_energy_production_mwh<-merge(merged_table,all_solar_production_mwh,id=year)
VA_annual_energy_production_mwh<-merge(VA_annual_energy_production_mwh,nuclear_production_mwh)

# Converting units to GWh
VA_annual_energy_production_gwh<-VA_annual_energy_production_mwh[,`:=`(total_energy_production=total_energy_production/1000,
...
VA_annual_energy_production_gwh<- VA_annual_energy_production_gwh%>%
  mutate(percent_renewable=((renewable_production/total_energy_production)*100))
...
VA_annual_energy_production_gwh<- VA_annual_energy_production_gwh%>%
  mutate(percent_carbon_free=((carbon_free_production/total_energy_production)*100))

# Graphing % of VA power production (in GWh/yr) from renewables & carbon-free sources 
percent_renewable_and_carbon_free<-VA_annual_energy_production_gwh[,c(1,7,9)]
...
carbon_free_generation_by_type<-VA_annual_energy_production_gwh[,c(1:5)]
...
annual_renewable_generation_by_type_line <- ggplot(data=melted_generation,mapping=aes(x=year,y=annual_energy_production_gwh,color=source_type, shape=source_type))+
  geom_line()+
  geom_point()+
  xlab("Year")+ylab("Annual VA Power Production (GWh)")+
  scale_color_discrete(name=NULL,breaks=c("total_energy_production",
                                          "nuclear_production",
                                          "hydro_production",
                                          "all_solar_production"),labels=c("Total Energy Production",
                                                                           ...
                                                                           "Hydro Production",
                                                                           "Solar Production"))+
  scale_shape_manual(name=NULL,breaks=c("total_energy_production",
                                        "nuclear_production",
                                        "hydro_production",
                                        "all_solar_production"),labels=c("Total Energy Production",
                                                                         "Nuclear Production",
...
annual_renewable_generation_by_type_line
ggsave(path=path2graphics, filename="annual_renewable_generation_by_type_line.png")

# Solar (broken into distributed and utility), Hydro, and Nuclear Generation over Time
year<-2001:2013
distrib_solar_production<-rep(0,13)
missing_distrib_solar_dt<-data.frame(year,distrib_solar_production)
distrib_solar_production_mwh<-rbind(distrib_solar_production_mwh,missing_distrib_solar_dt)
distrib_solar_production_mwh<-distrib_solar_production_mwh[order(-year)]
distrib_solar_production_gwh<-distrib_solar_production_mwh[,`:=`(distrib_solar_production=distrib_solar_production/1000)]

year<-2003:2015
utility_solar_production<-rep(0,13)
missing_utility_solar_dt<-data.frame(year,utility_solar_production)
utility_solar_production_mwh<-rbind(utility_solar_production_mwh,missing_utility_solar_dt)
utility_solar_production_mwh<-utility_solar_production_mwh[order(-year)]
utility_solar_production_gwh<-utility_solar_production_mwh[,`:=`(utility_solar_production=utility_solar_production/1000)]
VA_annual_energy_production_gwh<-merge(distrib_solar_production_gwh,VA_annual_energy_production_gwh,id=year)
VA_annual_energy_production_gwh<-merge(VA_annual_energy_production_gwh,utility_solar_production_gwh,id=year)


carbon_free_generation_by_type2<-VA_annual_energy_production_gwh[,c(1,2,3,4,6,11)]
...
annual_renewable_generation_by_type_line2 <- ggplot(data=melted_generation2,mapping=aes(x=year,y=annual_energy_production_gwh,color=source_type, shape=source_type))+
  geom_line()+
  geom_point()+
  xlab("Year")+ylab("Annual VA Power Production (GWh)")+
  scale_color_discrete(name=NULL,breaks=c("total_energy_production",
...
annual_renewable_generation_by_type_line2
ggsave(path=path2graphics, filename="annual_renewable_generation_by_type_line2.png")

#Stacked Annual Carbon Free Generation Broken Out by Type
carbon_free_generation_by_type3<-VA_annual_energy_production_gwh[,c(1,4,5,6)]
...
carbon_free_generation_by_type_stacked <- ggplot()+
  ...
  xlab("Year")+ylab("Annual VA Power Production (GWh)")+
  scale_fill_discrete(name=NULL,breaks=c("all_solar_production",
                                           "hydro_production",
                                          "nuclear_production"),labels=c("Solar Production",
                                                                          "Hydro Production",
                                                                          "Nuclear Production"))+
...
renewable_generation_by_type<-VA_annual_energy_production_gwh[,c(1,2,4,11)]
...
VA_annual_energy_production_gwh<-VA_annual_energy_production_gwh%>%
  mutate(non_renewable_production=(total_energy_production-renewable_production))
renewable_and_non_renewable<-VA_annual_energy_production_gwh[,c(1,7,12)]
...
VA_annual_energy_production_gwh<-VA_annual_energy_production_gwh%>%
  mutate(carbon_production=(total_energy_production-carbon_free_production))
carbon_and_carbon_free<-VA_annual_energy_production_gwh[,c(1,9,13)]
...
renewable_and_carbon_free<-VA_annual_energy_production_gwh[,c(1,3,7,9)]
